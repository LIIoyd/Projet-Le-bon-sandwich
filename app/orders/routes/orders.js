...
router.get("/", async (req, res, next) => {
  ...
  const schema = Joi.object({
    c: Joi.string().email() || undefined,
    sort: Joi.string().valid("date", "amount") || undefined,
    page: Joi.number().integer() || undefined,
  });
  ...
  if (!error) {
    ...
    try {
      ...
      if (client != undefined) {
        query = query.where("email", client);
    ...
    // Cas où il y a une erreur de validation
    res.sendStatus(400);
  }
});
