    ...
    if (order) {
      if (embed === "items") {
        const items = await axios.get(
          `http://node_orders:3000/orders/${id}/items`
        );
        if (items) {
...
router.post("/", async (req, res, next) => {
  ...
  const schema = Joi.object({
    ...
    items: Joi.array().items(
      Joi.object({
        uri: Joi.string()
          .pattern(new RegExp("^/sandwiches/[0-9]+$"))
          .required(),
  ...
  if (!error) {
    try {
      ...
      while (exist && req.body.items.length > ite) {
        id = req.body.items[ite].uri.replace("/sandwiches/", "");
        sandwich = await axios.get(
